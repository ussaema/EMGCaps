...
class SVM(ClassifierModel):
    ...
    def train(self, args, train_features, train_labels, valid_features = None, valid_labels = None, verbose=None):
        super().train(args, train_features, train_labels, valid_features, valid_labels)
        self.classifier = svm.SVC(kernel=self.svm_kernel)
        ...
        if valid_features is not None and valid_labels is not None:
            self.test(valid_features, valid_labels, verbose)
        self.save_log(self.full_model_path)

    def test(self, test_features, test_labels, verbose=None):
